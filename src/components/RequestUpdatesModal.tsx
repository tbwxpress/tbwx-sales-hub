    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="px-5 py-3 border-b border-border flex items-center justify-between">
          <h2 className="text-heading text-text">Request updates from {agentName}</h2>
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          {/* Filters */}
          <div>
            <div className="text-eyebrow text-dim mb-2">Filter by status</div>
          ...
          {/* Roster */}
          <div>
            <div className="text-eyebrow text-dim mb-2">Leads</div>
            <div className="border border-border rounded-md max-h-72 overflow-y-auto">
              {loading ? (
                <div className="p-4 text-body text-dim text-center">Loading roster…</div>
              ) : visibleLeads.length === 0 ? (
                <div className="p-4 text-body text-dim text-center">No leads in this agent&apos;s roster match the selected statuses.</div>
              ) : (
                <>
                  <div className="px-3 py-2 border-b border-border/50 flex items-center justify-between text-caption text-dim">
                  ...
                  {visibleLeads.map(lead => (
                    <label key={lead.row_number} className="flex items-center gap-3 px-3 py-2 hover:bg-elevated cursor-pointer border-b border-border/30 last:border-b-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-body text-text truncate">{lead.full_name}</div>
                        <div className="text-caption text-dim">{lead.lead_status} · {lead.city || '—'}</div>
                      </div>
                    </label>
                  ))}
                </>
          ...
          {/* Due date */}
          <div>
            <label className="text-eyebrow text-dim block mb-1">Due date</label>
            <input
              type="date"
              value={dueDate}
              onChange={e => setDueDate(e.target.value)}
              className="bg-elevated border border-border rounded-md px-2 py-1.5 text-body text-text"
            />
          </div>

          {/* Reason */}
          <div>
            <label className="text-eyebrow text-dim block mb-1">Reason (optional, ≤ 200 chars)</label>
            <textarea
              value={reason}
              onChange={e => setReason(e.target.value.slice(0, 200))}
              rows={2}
              placeholder="e.g. haven't heard back in 2 weeks"
              className="w-full bg-elevated border border-border rounded-md px-2 py-1.5 text-body text-text resize-none"
            />
          </div>
        ...
        <div className="px-4 py-3 border-t border-border flex items-center justify-end gap-2">
          <button onClick={onClose} className="text-body bg-elevated border border-border rounded-md px-3 py-1.5 text-muted hover:text-text">
            Cancel
          </button>
          <button
            onClick={submit}
            disabled={selected.size === 0 || sending}
            className="text-body bg-accent text-bg rounded-md px-3 py-1.5 font-medium disabled:opacity-50"
          >
