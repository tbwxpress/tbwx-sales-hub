...
export default function InboxPage() {
  ...
  // Template previews — "what did the customer actually see?" Body texts are
  // fetched from /api/templates once, on first expand, then cached for the
  // session. Keyed per message so multiple bubbles can be open at once.
  const [templateBodies, setTemplateBodies] = useState<Record<string, string> | null>(null)
  const [openTemplatePreviews, setOpenTemplatePreviews] = useState<Set<string>>(new Set())

  async function toggleTemplatePreview(msgKey: string) {
    if (templateBodies === null) {
      try {
        ...
        for (const t of (json?.data || [])) {
          if (t?.name) map[String(t.name)] = String(t.body || '')
        }
        setTemplateBodies(map)
      } catch {
        setTemplateBodies({}) // fetch failed — previews show a fallback note
      }
    }
    setOpenTemplatePreviews(prev => {
                      ...
                      return (
                        <div key={msg.id || idx}>
                          ...
                          <div className={`flex mb-1.5 ${msg.direction === 'sent' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            >
                              {/* Template badge — click to preview what the customer actually received */}
                              {msg.template_used && (() => {
                                const msgKey = String(msg.id ?? msg.wa_message_id ?? idx)
                                const isOpen = openTemplatePreviews.has(msgKey)
                                return (
                                  <>
                                    <button
                                      type="button"
                                      onClick={() => toggleTemplatePreview(msgKey)}
                                      className="text-[9px] text-accent/70 hover:text-accent mb-1 flex items-center gap-1 cursor-pointer"
                                      title="Show the actual template text the customer received"
                                    >
                                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                                      </svg>
                                      Template: {msg.template_used}
                                      <span className="underline decoration-dotted">{isOpen ? 'hide' : 'preview'}</span>
                                    </button>
                                    {isOpen && (
                                      <div className="mb-1.5 rounded-md px-2 py-1.5 text-[11px] whitespace-pre-wrap border border-white/10 bg-black/20 text-wa-text/90">
                                        {templateBodies === null
                                          ? 'Loading…'
                                          : templateBodies[msg.template_used]
                                            || 'Template text unavailable (deleted or renamed in WhatsApp Manager).'}
                                      </div>
                                    )}
                                  </>
                                )
